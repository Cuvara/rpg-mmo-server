    ...
    internal static byte[] Write(SnapshotMessage m)
    {
        ...
        using (var w = new Utf8JsonWriter(buffer))
        {
            ...
            for (int i = 0; i < m.Entities.Count; i++)
            {
                ...
                // Omitted when zero for the same reason as the two above: zero is the
                // reserved "not sent" value, and writing it explicitly would assert that
                // this sender has a retrigger counter whose value is the reserved one.
                //
