...
// TestClientConnClearIdentityClaimsOnce pins the property cleanupSession relies
// on: only one caller can ever take the identity, so concurrent teardown paths
// (an explicit MsgDisconnect and handleConn's deferred cleanup) cannot both
// destroy the same session.
func TestClientConnClearIdentityClaimsOnce(t *testing.T) {
	cc := NewClientConn(nil, logger.New("error"), ratelimit.Bucket{})
	cc.SetAuthenticated("user1")
	...
	if claims != 1 {
		t.Errorf("identity claimed %d times, want exactly 1", claims)
	}
}
