...
| C4 | ~~**ADR-14 stages 7 and 8.**~~ **DONE 2026-09-13.** Stage 7: `deploy/k8s/app/70-fleetautoscaler-dungeon.yaml`, Buffer 2 / 2-6 on a 30s sync, on the **dungeon** fleet — the first fleet whose spare Ready pods claim no world (ADR-18 decision 4, ADR-26 decision 8). The map fleet's prohibition is unchanged; `verify.sh`'s `cluster.autoscaler` needed no loosening to admit it (a template with no `GAMESERVER_MAP_ID` already read as unpinned) but did need **reach** — it inspected only the fleet a target names, and now sweeps every fleet in `VERIFY_NAMESPACES`. Proven offline on both fleet shapes by `verify/tests/autoscaler_rule_test.sh`: FAIL for the map-pinned one, PASS for the map-less one. Stage 8: the Go-image manifests were already deleted (`6281c72`) and `map-servers-dev` / `dungeon-servers-dev` are gone from `k3d-rpg-dev`; the three files left in `deploy/agones/` are the C# dev fleet that `rollback-to-compose.sh` restores, so they were **documented as the rollback target rather than deleted**. | S |
| C5 | ~~**The client half of C1 and C2.**~~ **DONE 2026-09-13, proven with two real built players.** netcode v0.37.0 carries `party_id` and `ConnectToDungeonAsync` (and replays the party on reconnect); `Scripts/Nakama/Social/PartyService.cs` wraps the four Nakama RPCs; the session flow gains a party step before the world and **fails loudly** on a dungeon with no party rather than falling back to a map. Measured: two Windows players, one creating the party and one joining it by id, **both landed on `127.0.0.1:7019`** and the instance reported `players_online: 2`. | M |
...
> **RESOLVED 2026-09-13.** The client half was built and proven the next day: see C5. What
> follows is kept because the lesson outlives the gap.
>
...
C3 (Android) and C4 (ADR-14 stages 7-8) are both done now, and neither changed the shape of
any flow content sits on: an Android client joins the same way a Windows one does — measured,
...
**The gate is now open on both sides, and this is what that means concretely.** On
2026-09-13 two *built Unity players* — not a probe — created a party through Nakama, joined it
by id, and both entered the same dungeon instance:

```
c1: [DOTSNet] party ready: 8fadf042dfd21315cae10651ad3284c2   -> 127.0.0.1:7019
c2: [DOTSNet] party ready: 8fadf042dfd21315cae10651ad3284c2   -> 127.0.0.1:7019
server: "players_online":2  "sealed_cipher":"chacha20-poly1305"
```

Both were refused on their first join for not sealing and escalated themselves, which is the
shipped default. **What content can now be written against.** An open-world map session and a party-instanced
