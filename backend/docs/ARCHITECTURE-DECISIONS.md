...
**Updated 2026-09-24: ON for dev (2026-09-13) and staging (2026-09-24), each with its own pinned self-signed pair; OFF for production until it has a host (#425).** Enabling it on staging surfaced two procedure gaps, both fixed: Nakama must be restarted after the opt-in (a ConfigMap change does not restart a pod), and the verification suite read the gateway pin from dev's run directory rather than from the target cluster.
