...
type SealedServerHello struct {
	...
	// ServerSignature is a 64-byte Ed25519 signature over
	//
	//	"cuvara/sealed-identity/v1" || 0x00 || transcript || 0x00 || identity_public
	//
	// made with the server's PER-POD identity key (ADR-25). The transcript
	// inside it is byte-identical to the one Binding covers — this wraps that
	// transcript, it does not change it, so ADR-22's vectors stay valid.
	//
	// It replaces Binding for real clients. Binding is a symmetric MAC under
	// JOIN_TOKEN_SECRET, so a client able to check it could forge join tokens;
	// this needs only the public half, which the gateway delivers in
	// EnterWorldResponse.ServerPublicKey. Both fields are sent: Binding still
	// serves the harnesses that hold the secret, and giving one field number two
	// meanings is how two versions silently disagree about a byte.
...
type EnterWorldResponse struct {
	...
	// ServerPublicKey is the target game server's 32-byte Ed25519 IDENTITY key,
	// generated by that pod at startup and carried here from its registry entry
	// (ADR-25). It verifies SealedServerHello.ServerSignature on the gameplay
	// hop.
	//
	// It adds no new trust relationship — the client already takes ServerAddr
	// from this message on the same authority, and an attacker who can rewrite
	// ServerAddr redirects the player anyway. It also inherits that authority
	// exactly: this hop is plaintext in every environment today, so a client
	// that checks a signature against this key has learned that the peer holds
	// THIS key, not that this key is the real server's. See the schema comment
	// on wire.proto's server_public_key, and sealed.ClientResult.
	//
	// Empty when the registry entry has no key: an older game server, or an
	// entry written before ADR-25.
