...
| `GAMESERVER_PUBLIC_ADDR` | `:<gameserver port>` | **The address handed to clients** in `MsgEnterWorldResp.ServerAddr`. The default is listen-style, which a client normalizes to *its own* loopback — correct only when the client is on the same box. On a VPS this must be `<public-host-or-ip>:<port>`. This is the single most common misconfiguration. |
