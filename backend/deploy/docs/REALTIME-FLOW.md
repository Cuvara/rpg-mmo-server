...
> **Status: historical analysis, 2026-08-17.** This traces the enter-map flow as it
> stood *before* the Agones address work landed, and its diagnosis section is what
> justified that work. **The central defect it identifies is fixed.** The game server
> no longer advertises a hostless `:9000` under Agones: it reads the assigned port from
> the sidecar's GameServer status and composes it with `GAMESERVER_ADVERTISE_HOST`,
> because `status.address` is the node address and is not dialable by a client. Read
> **ADR-16** for what shipped and **ADR-15** for the costing. Everything here about the
> compose path, the handshake and the file-by-file trace still holds; treat the "what
> breaks" tables as a record of what *was* broken.
