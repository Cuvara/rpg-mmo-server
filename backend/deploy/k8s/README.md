...
built locally (or in CI) and piped into the k3d node's containerd store. This
is why every manifest says `imagePullPolicy: IfNotPresent` and not `Always`.

For multi-node clusters or any environment where k3d import is not possible,
push images to a real registry with `registry/push.sh`:

```bash
REGISTRY=ghcr.io/cuvara TAG=$(git rev-parse --short HEAD) ./registry/push.sh
```

All workload manifests (gateway, nakama, fleet) carry an `imagePullSecrets`
reference to `registry-creds`. The Secret is optional: when it does not exist
the kubelet uses anonymous pulls (fine for public registries or k3d import).
When it does exist, the kubelet authenticates. Create it per namespace:
...
See `registry/README.md` for the full workflow.

## The initdb ConfigMap

The `gamestate-initdb` ConfigMap is generated by `data/apply.sh` from
`deploy/db/init-gamestate.sql` -- not committed as a filled manifest. kustomize
refuses file sources above the kustomization root, and copying the SQL into
`data/` would create a third copy of a schema that two tests already pin to
two existing copies.

`data/configmap-init-gamestate.yaml` documents the manifest shape.
`data/job-init-gamestate.yaml` is a one-shot Job for applying the schema to an
existing database that missed the first-boot initdb (adopted PVC, restore).

On fresh deploys, `apply.sh` creates the ConfigMap before the StatefulSet, and
postgres runs `/docker-entrypoint-initdb.d/*` exactly once on an empty PGDATA.
The game server's built-in migrator is the authoritative schema path for
upgrades.

## Nakama Plugin Packaging

The Nakama image includes the Go runtime plugin **baked in**, not delivered via
an initContainer or a volume mount. The plugin is ABI-locked to the exact Go
toolchain and nakama-common version in the server binary, so the two must be
versioned as one artifact.

Build with `docker/Dockerfile.nakama` (or the equivalent
`nakama-plugin.Dockerfile` at the deploy root):

```bash
cd backend
docker build -f deploy/docker/Dockerfile.nakama \
  --build-arg GIT_REVISION=$(git rev-parse HEAD) \
  -t rpg-mmo/nakama:3.40.0 .
```

The `data/nakama.yaml` Deployment references `rpg-mmo/nakama:3.40.0` with
`imagePullPolicy: IfNotPresent`. For k3d, import the image after building.
For a real registry, push with `registry/push.sh`.
