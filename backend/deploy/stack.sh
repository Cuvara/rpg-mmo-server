...
# GATEWAY_TLS_CERT/KEY are here for exactly the reason described above, and the
# reason is not hypothetical: .env.example now WRITES both names (empty), so a
# .env derived from it -- or generated by CD -- mentions them. Without this list
# the file's empty value would clobber
#
#   GATEWAY_TLS_CERT=... GATEWAY_TLS_KEY=... ./stack.sh up
#
