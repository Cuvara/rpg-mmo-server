...
| AOI | **Uniform spatial grid, gated on population spread** — brute-force scan below the gate | **Both, and the gate picks.** The grid is rebuilt once per gather scope with the `EntityView` composed into it; queries take it only when the population occupies >= 96 cells, and matches are reordered by sorting an **int permutation** rather than the view structs. **1.5-2.25x on the stock map and 0.85-1.5x on clustered hotspot layouts** — one modelled clustered layout still reads ~0.86x and is recorded in Part XI rather than tuned away. Two earlier attempts got this wrong in opposite directions: the first index lost 2.8x and was reverted (Part V); the second shipped a struct sort that made it **1.3-2.3x slower on clustered maps** while the gate waved it through (Part XI). Read `backend/docs/BENCHMARK.md` **Part XI first**, then X and V, before proposing any further AOI work |
